...
local posnode=minetest.get_node(pos)
	...
	local pistonspec = minetest.registered_nodes[nn.name].mesecons_piston
if pistonspec~=nil then
	if pistonspec.sticky then
		local ddir = piston_get_direction(pistonspec.dir, nn)
if ddir~=nil and dir~=nil then
if (ddir.x==dir.x or ddir.x==0) and (ddir.y==dir.y or ddir.y==0) and (ddir.z==dir.z or ddir.z==0) then
local 		pullpos = np
		if nn.name=="mesecons_pistons:piston_sticky_on" then
 		pullpos = {x=pullpos.x+dir.x, y=pullpos.y+dir.y,z=pullpos.z+dir.z}
end
