...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		local cl=meta:get_string("cloud")
		if cl==nil then return end
		if tonumber(cl)<1 then
		minetest.set_node(pos, {name = "air"})
		else
		meta:set_string("cloud",tostring(tonumber(cl)-1))
		end
	end
})
...
minetest.register_node("default:leaves", {
	...
	drop = {
		...
		items = {
			...
			{
				-- player will get sapling with 1/20 chance
				items = {'moretrees:apple_tree_sapling'},
				rarity = 20000,
			},
			{
				-- player will get leaves only if he get no saplings,
...
minetest.register_node("default:cloud", {
	...
	after_place_node = function(pos, placer)
		local meta = minetest.get_meta(pos)
		meta:set_string("cloud", tostring(math.floor(math.random(3,17))))
	end,
...
local function has_locked_chest_privilege(meta, player)
	...
	if player:get_player_name() ~= meta:get_string("owner")  and (not minetest.check_player_privs(player:get_player_name(), {delprotect=true}) or not keys["jump"]==true ) then
		return false
	end
	return true
end
...
function default.furnace_active(pos, percent, item_percent)
    local formspec = 
		...
		"button_exit[0,0;1,0.8;close_me;ESC]"..
...
minetest.register_node("default:apple", {
	...
	on_use = minetest.item_eat(1.102),
	sounds = default.node_sound_leaves_defaults(),
...
minetest.register_node("default:grass_1", {
	...
	is_ground_content = true,
	buildable_to = true,
	groups = {snappy=3,flammable=3,grass=1,flora=1,attached_node=1},
	sounds = default.node_sound_leaves_defaults(),
	selection_box = {
		type = "fixed",
		fixed = {-0.5, -0.5, -0.5, 0.5, -5/16, 0.5},
