...
function technic.register_solar_array(data)
	...
	minetest.register_abm({
		...
		local cloud=data.power
		if pos1.y>130 then 
local rnd=math.floor(pos.z/11)%11+math.floor(pos.x/7)%7+math.floor((pos.x*pos.z)/3)%3
cloud=(data.power*(40+rnd))/40
 end
			local charge_to_give = math.floor((light + pos1.y/50) * cloud)
