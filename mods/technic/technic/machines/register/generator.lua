...
function technic.register_generator(data) 
	...
	local generator_formspec =
		...
		"image[4,2;1,1;default_furnace_fire_bg.png]"..
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local buffer=meta:get_int("internal_EU_charge")
			local demand=meta:get_int(tier.."_EU_demand")
if demand==nil then
demand=0
end
if demand==nil then
buffer=0
			...
			if burn_time == 0 and (demand>0 or buffer>burn_totaltime*150) then
				local inv = meta:get_inventory()
