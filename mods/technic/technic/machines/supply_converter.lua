...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if from and to then
		demand = meta:get_int(to.."_EU_demand")
		if demand==nil then
			demand = 9000
		end
			technic.switching_station_timeout_count(pos, from)
			local input = meta:get_int(from.."_EU_input")
			meta:set_int(from.."_EU_demand", math.floor((demand/remain+99)/100)*100)
			meta:set_int(from.."_EU_supply", 0)
--			meta:set_int(to.."_EU_demand", 0)
			meta:set_int(to.."_EU_supply", input)
			meta:set_string("infotext", machine_name
				.." ("..input.." "..from.." -> "
				..tostring(math.floor((input*remain)/100)*100).." "..to..")")
		else
