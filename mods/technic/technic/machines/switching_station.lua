	...
	elseif machines[name] then
		...
		elseif machines[name] == technic.producer_receiver then
			...
			add_new_cable_node(RE_nodes, pos)
				...
				if technic.is_tier_cable(minetest.get_node({x=pos.x,y=pos.y+1,z=pos.z}).name, "MV") then
				add_new_cable_node(RE_nodes, pos)
				else
				add_new_cable_node(PR_nodes, pos)
				end
...
--minetest.log("action",minetest.get_node(pos1).name)
--minetest.log("action",tostring(meta1:get_int(eu_demand_str)))
		RE_eu_demand = RE_eu_demand + meta1:get_int(eu_demand_str)
		end
--		minetest.log("action","Total RE demand:"..RE_eu_demand)
		...
		for _, pos1 in pairs(BA_nodes) do
			...
			local max_charge=40000
			local name1=minetest.get_node(pos1).name
			if name1=="technic:mv_battery_box" then
			max_charge=200000
			end
			if name1=="technic:hv_battery_box" then
			max_charge=1000000
			end
			if charge<100 then
 				RE_eu_demand=RE_eu_demand+(max_charge/200)
			end
			BA_eu_demand = BA_eu_demand + meta1:get_int(eu_demand_str)
			if charge<max_charge-20000 then
			loading = loading + meta1:get_int(eu_demand_str)
			else
			notloading = notloading + (max_charge-charge)
			end
		end
		...
		for _, pos1 in pairs(PR_nodes) do
			...
			local dem=math.floor(RE_eu_demand+loading)
			if minetest.get_node(pos1).name~="technic:supply_converter" then 				meta1:set_int(eu_demand_str, dem)
			meta1:set_int("internal_EU_charge",math.floor(notloading))
			end
		end
		...
		for _, pos1 in pairs(PR_nodes) do
			if minetest.get_node(pos1).name=="technic:supply_converter" then
			meta1 = minetest.get_meta(pos1)
			local dem=math.floor(RE_eu_demand-PR_eu_supply+loading)
			local crr=meta1:get_int(eu_demand_str)
			if crr==nil then
			crr=0
			end	
			if math.floor((dem+99)/100)*100>crr then
			dem=crr+100
			elseif math.floor((dem+99)/100)*100<crr then
			dem=crr-100
			end
local sp=meta1:get_int(eu_supply_str)
if dem>sp then
if math.random(1,9)==1 then
dem=math.floor(((RE_eu_demand-PR_eu_supply)/0.9)/100)*100
else
dem=math.floor(((RE_eu_demand-PR_eu_supply+loading)/0.9)/100)*100
end
end
if dem<0 then
dem=0
end
			meta1:set_int(eu_demand_str, dem)
			meta1:set_int("internal_EU_charge",math.floor(notloading))
end
end
		...
		if PR_eu_supply + BA_eu_supply >= RE_eu_demand then
			...
			local charge_factor = 1 -- Assume all batteries depleted
			if BA_eu_supply > 0 then
				charge_factor = (PR_eu_supply +BA_eu_supply/200 - RE_eu_demand) / BA_eu_supply
			end
			for n,pos1 in pairs(BA_nodes) do
				meta1 = minetest.get_meta(pos1)
				local eu_supply = meta1:get_int(eu_supply_str)
				meta1:set_int(eu_input_str, math.floor(eu_supply * charge_factor))
			local chr=meta1:get_int("internal_EU_charge")
			meta1:set_int("internal_EU_charge",chr+math.floor(eu_supply * charge_factor))
		...
		local charge_factor = 0 -- Assume all batteries fully charged
		if BA_eu_demand > 0 then
			charge_factor = (PR_eu_supply +BA_eu_supply/200)/ BA_eu_demand
		end
		for n, pos1 in pairs(BA_nodes) do
			meta1 = minetest.get_meta(pos1)
			local eu_demand = meta1:get_int(eu_demand_str)
