...
minetest.register_craft({
	...
	recipe = {
			{'default:mese',         'technic:carbon_steel_block', 'default:mese'        },
...
vi = vi + 1
	...
	vm:update_liquids()
	vm:write_to_map()
 if (not active or first) and math.random(1,5)==3 then
 vm:update_map()
 end
end
...
local formspec = "size[5,3.25]"..
		 "button_exit[3.5,0;1,0.8;button1;ESC]"..

"field[0.3,1.5;2,1;range;"..S("Range")..";"..meta:get_int("range").."]"
-- The names for these toggle buttons are explicit about which
-- state they'll switch to, so that multiple presses (arising
-- from the ambiguity between lag and a missed press) only make
-- the single change that the user expects.
if meta:get_int("shape") == 0 then
formspec = formspec.."button[3,1.2;2,1;shape1;"..S("Sphere").."]"
else
formspec = formspec.."button[3,1.2;2,1;shape0;"..S("Cube").."]"
end
if meta:get_int("mesecon_mode") == 0 then
formspec = formspec.."button[0,2;5,1;mesecon_mode_1;"..S("Ignoring Mesecon Signal").."]"
else
formspec = formspec.."button[0,2;5,1;mesecon_mode_0;"..S("Controlled by Mesecon Signal").."]"
end
if meta:get_int("enabled") == 0 then
formspec = formspec.."button[0,2.75;5,1;enable;"..S("%s Disabled"):format(S("%s Forcefield Emitter"):format("HV")).."]"
else
formspec = formspec.."button[0,2.75;5,1;disable;"..S("%s Enabled"):format(S("%s Forcefield Emitter"):format("HV")).."]"
end
