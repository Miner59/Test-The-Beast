...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local itemname=string.lower(srcstack:get_name())
		local itemdesc=""
		if not srcstack:is_empty() or srcstack:get_metadata()~=nil then --all items with metadata are not repairable
			...
			if itemdef and
					...
					itemdesc=string.lower(itemdef.description)
				repairable = true
			end
		end
		...
		elseif eu_input >= demand then
			...
			local repair=-500
			if string.find(itemname,"mese") or string.find(itemdesc,"mese") then
			local repair=-300
			elseif string.find(itemname,"dia") or string.find(itemdesc,"dia") then
			local repair=-200
			elseif string.find(itemname,"mithril") or string.find(itemdesc,"mithril") then
			local repair=-100
			end
			srcstack:add_wear(repair)
