...
function write_to_cgpsmap(itemstack, user)
  --print("write_to_cgpsmap")
	...
	local formspec = "size[9,10;]"..
			"button_exit[2,1;5,0.5;write;Write to cgpsmap]"..
			"textlist[0,2.0;9,6;bookmark_list;"..list..";"..bkmrkidx.."]"..
			 "button_exit[0,9;1,0.8;button1;ESC]"
...
function read_from_cgpsmap(itemstack, user, meta)
  --print("read_from_cgpsmap")
	local formspec = "size[9,5]"..
			...
			.. "button_exit[7,0;1,0.8;button1;ESC]"
	...
	selected_cgpsmap[user:get_player_name()] = itemstack
      ...
      formspec=formspec.."label[2,0.5;bookmark pos: ("..meta["x"]..","..meta["y"]..","..meta["z"]..")]"..
...
minetest.register_craftitem("compassgps:cgpsmap", {
	...
	inventory_image = "cgpsmap_blank.png",
	groups = {book = 1},
	stack_max = 1,
...
--	if bkmrk.bkmrkname==nil or bkmrk.bkmrkname=="" or bkmrk.bkmrkname=="current position" then
--	  bkmrk.bkmrkname=playername.."'s map"
--	end
	...
	if (formname == "compassgps:logbook") then
		if not player then
			return
		end
		...
		local meta=minetest.deserialize(itemstack:get_metadata())
		if meta==nil then
			if cgps_trip_nr[player:get_player_name()]==nil then
				cgps_trip_nr[player:get_player_name()]=1
			end
			local data={}
			data.tripname="Trip"..cgps_trip_nr[player:get_player_name()]
			data.nr=cgps_trip_nr[player:get_player_name()]
			data.bookmarks={}
			itemstack:set_metadata(minetest.serialize(data))
		       itemstack:set_wear(1)
		else
			meta.nr=cgps_trip_nr[player:get_player_name()]
local lastnr=1
if meta.bookmarks~=nil then
local owner=false
for i,bookmark in pairs(meta.bookmarks) do
if bookmark~=nil then
if not owner then
if bookmark.player==player:get_player_name() then
owner=true
end
end
else
...
if tonumber(i)>lastnr then
lastnr=tonumber(i)
end
end
if lastnr>=30 then return end
end
		       itemstack:set_wear(1+(lastnr-1)*(65535/30))
		end
		...
		local meta=minetest.deserialize(itemstack:get_metadata())
		if meta~=nil then
			if cgps_trip_nr[player:get_player_name()]==meta.nr then
				cgps_trip_nr[player:get_player_name()]=meta.nr+1
			end
		end
		       itemstack:set_wear(0)
			player:set_wielded_item(itemstack)
		end
		if fields["tripname"] and fields["tripname"]~="" then
		local itemstack=player:get_wielded_item()
		local meta=minetest.deserialize(itemstack:get_metadata())
		if meta==nil then
			if cgps_trip_nr[player:get_player_name()]==nil then
...
--minetest.register_craft({
--	output = 'compassgps:logbook',
--	type="shapeless",
--	recipe = 
--		{'default:book', 'compassgps:cgpsmap', 'compassgps:cgpsmap'}
	
--})

minetest.register_tool("compassgps:logbook", {
	description = "Travel Logbook",
	inventory_image = "cgpsmap_itinerary.png",
	groups = {book = 1},
	stack_max = 1,
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local meta = minetest.deserialize(itemstack:get_metadata())
local  list,bkmrkidx=compassgps.bookmark_loop("I",user:get_player_name(),meta)
if bkmrkidx==nil then
bkmrkidx=""
end
local tnr=cgps_trip_nr[user:get_player_name()]
if tnr==nil then
tnr="1"
end
local tripname="Trip"..tnr
if meta~=nil then
minetest.log("action",itemstack:get_metadata())
if meta.tripname~=nil then
tripname=meta.tripname
end
end
		local formspec="size[17,11.8]"
			.."button_exit[16,11;1,0.8;button1;ESC]"
			.."label[0,0;List entries:]"
			.."textlist[0,0.2;8,11.6;list;"..list..";"..bkmrkidx.."]"
			.."label[8.25,11;Journey name:]"
			.."field[10.55,11.2;1.75,1;tripname;;"..tripname.."]"
if bkmrkidx=="" and itemstack:get_wear()==0 then
formspec=formspec.."button_exit[12.1,11;3.85,0.8;start;Start writing to this logbook]"
elseif itemstack:get_wear()>0 and tonumber(bkmrkidx)<30 then
formspec=formspec.."button_exit[12.1,11;3.85,0.8;stop;Stop writing to this logbook]"
elseif bkmrkidx>=30 then
formspec=formspec.."label[12.1,11;No space left in this logbook]"
else
formspec=formspec.."button_exit[11.85,11;4.3,0.8;start;Continue writing to this logbook]"
end
		minetest.show_formspec(user:get_player_name(), "compassgps:logbook", formspec)
