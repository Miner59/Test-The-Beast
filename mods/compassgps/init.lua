...
local hud_default_x=0.36
local hud_default_y=0.025
local hud_default_color="FFFFFF"
local compass_default_type="c"
local compass_valid_types={"c"}
local log_dist=200 --enter a new bookmark to the travel logbook after 200 blocks distance from last bookmark
local going_back_dist=50 --insert a new bookmark after 50 blocks from other bookmarks
...
for name,stng in pairs(settings) do
  ...
  if settings[name].trip_nr then
    cgps_trip_nr[name]=settings[name].trip_nr
  end
...
if mode=="I" then
textlist_logbook[playername]={}
local meta=findidx
if meta==nil then
minetest.log("action","1")
return ""
end
if meta.bookmarks==nil then
minetest.log("action","2")
return ""
end
local kmm=""
  for i=1,31 do
	local bookmrk=meta.bookmarks[tostring(i)]
	if bookmrk==nil then break end
      list = list..kmm..compassgps.bookmark_name_string(bookmrk).." : "..compassgps.pos_to_string(bookmrk)
      kmm=","
      textlist_logbook[playername][i]=bookmrk
minetest.log("action",list)
end
else
    ...
    elseif fields["remove_bookmark"] and textlist_clicked[playername] then
      ...
      if textlist_bkmrks[playername][bkmrkidx].player ~= playername then
        ...
        if not player_privs["privs"] then
          minetest.chat_send_player(playername,"you can not remove someone elses bookmark:"..
...
function compassgps.set_bookmark(playername, bkmrkname, type, predefinedpos)
 ...
 pos = predefinedpos
  ...
  bkmrkname=compassgps.clean_string(bkmrkname)
	...
	if bkmrkname == "" then
		minetest.chat_send_player(playername, "Give the bookmark a name.")
		return
	end
	if bkmrkname == "default" or bkmrkname == "bed" or bkmrkname == "sethome"
       ...
       or string.sub(bkmrkname,1,7)=="*admin*" then
		minetest.chat_send_player(playername, "A bookmark with the name '"..bkmrkname.."' can't be created.")
		return
	end
	if bookmarks[playername..bkmrkname] then
		minetest.chat_send_player(playername, "You already have a bookmark with that name.")
		return
	end
  ...
  if pos.type=="S" and compassgps.count_shared(playername) >= max_shared then
		minetest.chat_send_player(playername, "The maximum number of shared bookmarks any user can create is "..max_shared)
    return
	end
  ...
  compassgps.write_bookmarks()
	minetest.chat_send_player(playername, "Bookmark "..bkmrkname.." added at "..compassgps.pos_to_string(pos).." type="..pos.type)
end
...
function compassgps.verify_bookmark_parms(from_function,player,playername,bkmrkidx)
	...
	if not player then
    print("compassgps."..from_function.." player not found")
    if not playername then print("  playername=nil")
    else print("  playername="..playername)
    end --if not playername
    return false
  end --if not player
  if not tonumber(bkmrkidx) then
    print("compassgps."..from_function.." invalid bkrmkidx")
    if not bkmrkidx then print("  bkmrkidx=nil")
...
function compassgps.remove_bookmark(playername, bkmrkidx)
  ...
  print("remove bookmark playername="..playername.." bkmrkidx="..bkmrkidx)
	minetest.chat_send_player(playername, "removed "..
      compassgps.bookmark_name_string(textlist_bkmrks[playername][bkmrkidx]))
...
function compassgps.remove_bookmark_byname(playername, bkmrkname)
	...
	if not bookmarks[playername..bkmrkname] then
		minetest.chat_send_player(playername, "You have no bookmark with this name.")
		return
	end
	bookmarks[playername..bkmrkname] = nil
  compassgps.write_bookmarks()
	minetest.chat_send_player(playername, "The bookmark "..bkmrkname.." has been successfully removed.")
end
...
function compassgps.teleport_bookmark(playername, bkmrkidx)
  ...
  if not compassgps.verify_bookmark_parms("teleport_bookmark",player,playername,bkmrkidx)
      then return end
		local itemstack = player:get_wielded_item():get_name()
if not string.sub(itemstack, 0, 11) == "compassgps:" then
return
end
local pos=player:getpos()
local to=textlist_bkmrks[playername][bkmrkidx]
itemstack=player:get_wielded_item()
local wear=itemstack:get_wear()
itemstack:set_wear(wear+math.floor((math.abs(pos.x-to.x)+math.abs(pos.y-to.y)+math.abs(pos.z-to.z))/3))
        player:set_wielded_item(itemstack)

  print("compassgps teleporting player "..playername.." to "..
      compassgps.bookmark_name_string(textlist_bkmrks[playername][bkmrkidx]))
	minetest.chat_send_player(playername, "Teleporting to "..
      compassgps.bookmark_name_string(textlist_bkmrks[playername][bkmrkidx]))
...
function compassgps.find_bookmark_byname(playername, bkmrkname)
	...
	if not bkmrkname or bkmrkname == "" then
		minetest.chat_send_player(playername, "No bookmark was specified.")
		return
	end
	if bkmrkname == "default" then
		minetest.chat_send_player(playername, "Pointing at default location.")
	...
	minetest.chat_send_player(playername, "Pointing at "..bkmrkname..".")
end
...
function compassgps.find_bookmark(playername, bkmrkidx)
  ...
  if not compassgps.verify_bookmark_parms("find_bookmark",player,playername,bkmrkidx)
	...
	minetest.chat_send_player(playername, "Pointing at "..point_to[playername].bkmrkname)
end
...
minetest.register_globalstep(function(dtime)
   local itinerary_update=false
   local time=os.date("%d/%m/%y %H:%M")
	if last_time_spawns_read ~= time then
		last_time_spawns_read = time
		read_spawns()
		itinerary_update=true
	end
	local players  = minetest.get_connected_players()
	for i,player in ipairs(players) do
    local playername = player:get_player_name();
	...
	if string.sub(wielded_item, 0, 11) == "compassgps:" and string.sub(wielded_item, 0, 18) ~= "compassgps:cgpsmap" and string.sub(wielded_item, 0, 18) ~= "compassgps:logbook"then
      --if the player is wielding a compass, change the wielded image
	wear=player:get_wielded_item():get_wear()
	if wear+1 < 65535 then
      wielded=true
...
if string.sub(stack:get_name(), 0, 18) == "compassgps:logbook" then
if itinerary_update and stack:get_wear()>0 then
local ppos=player:getpos()
local meta=stack:get_metadata()
local data={}
if meta==nil or meta=="" then
if cgps_trip_nr[playername]==nil then
cgps_trip_nr[playername]=1
end
data.bookmarks={}
data.tripname="Trip"..cgps_trip_nr[playername]
data.nr=cgps_trip_nr[playername]
local new_bookmark={x=ppos.x,y=ppos.y,z=ppos.z,player=playername,type="I",bkmrkname=data.tripname.." Bkmrk.1 ("..tostring(time)..")" }
data.bookmarks["1"]=new_bookmark
else
data=minetest.deserialize(meta)
local lastnr=1
if data.tripname==nil then
if cgps_trip_nr[playername]==nil then
cgps_trip_nr[playername]=1
end
data.tripname="Trip"..cgps_trip_nr[playername]
end
if data.bookmarks==nil then
data.bookmarks={}
end
if data.bookmarks["1"]==nil then
local new_bookmark={x=ppos.x,y=ppos.y,z=ppos.z,player=playername,type="I",bkmrkname=data.tripname.." Bkmrk.1 ("..tostring(time)..")" }
data.bookmarks["1"]=new_bookmark
else
local add_bookmark=true
for i,bookmark in pairs(data.bookmarks) do
if math.abs(ppos.x-bookmark.x)<going_back_dist or math.abs(ppos.y-bookmark.y)<going_back_dist*3 or math.abs(ppos.z-bookmark.z)<going_back_dist then
add_bookmark=false
break
end
if lastnr<i then
...
if add_bookmark then
local bookmark=data.bookmarks[tostring(lastnr)]
if bookmark==nil or bookmark=="" then
local new_bookmark={x=ppos.x,y=ppos.y,z=ppos.z,player=playername,type="I",bkmrkname=data.tripname.." Bkmrk."..tostring(lastnr+1).." ("..tostring(time)..")" }
data.bookmarks[tostring(lastnr+1)]=new_bookmark
elseif math.abs(ppos.x-bookmark.x)>log_dist or math.abs(ppos.y-bookmark.y)>log_dist*3 or math.abs(ppos.z-bookmark.z)>log_dist then
local new_bookmark={x=ppos.x,y=ppos.y,z=ppos.z,player=playername,type="I",bkmrkname=data.tripname.." Bkmrk."..tostring(lastnr+1).." ("..tostring(time)..")" }
data.bookmarks[tostring(lastnr+1)]=new_bookmark
else
return
...
if wear+65535/30>65535 then
stack:set_wear(0)
			if cgps_trip_nr[playername]==nil then
				cgps_trip_nr[playername]=1
			else
			if meta.nr==cgps_trip_nr[playername] then
				cgps_trip_nr[playername]=cgps_trip_nr[playername]+1 --this trip is finished, increase trip number
			end			
			end
else
stack:set_wear(math.floor(wear+65535/30))
end
end
...
elseif i<=activewidth and string.sub(stack:get_name(), 0, 11) == "compassgps:" and string.sub(stack:get_name(),0,18) ~= "compassgps:cgpsmap"   then
	wear=stack:get_wear()
	if wear+1 < 65535 then
        ...
        player:get_inventory():set_stack("main",stackidx,ItemStack(nil))
	end
					break
					end
    ...
    if gotacompass then
      ...
      --update compass image to point at target
  		if wielded then
local itemstack=ItemStack("compassgps:"..
            compassgps.compass_type_name(playername,compass_image))
itemstack:set_wear(wear+1)
        player:set_wielded_item(itemstack)
...
itemstack:set_wear(wear)
        ...
        player:get_inventory():set_stack("main",stackidx,itemstack)
...
--  if player_privs["teleport"] then
    telebutton="button[4,9.3;3.5,1;teleport;Teleport to bookmark]"
--  end
  local sharedbutton=""
  if player_privs["settime"] and not singleplayer then
    sharedbutton="button[2.3,0.7;2.8,1;new_shared_bookmark;Create shared]"
  end
  local adminbutton=""
  if player_privs["privs"] and not singleplayer then
    adminbutton="button[4.6,0.7;2.8,1;new_admin_bookmark;Create admin]"
  end

  local checkboxes=""
  if not singleplayer then
    checkboxes="label[3.65,1.75;Show:]"..
    "checkbox[4.35,1.4;show_private;Private;"..view_type_P[name].."]"..
    "checkbox[4.35,1.7;show_shared;Shared;"..view_type_S[name].."]"..
    ...
    "button[5.5,0;2.35,0.8;settings;Settings]"..
		"button[0,0.7;2.8,1;new_bookmark;Create bookmark]"..
    sharedbutton..
    adminbutton..
    "button[6.5,0.7;2.7,1;remove_bookmark;Remove bookmark]"..
    "label[0,1.75;Sort by:]"..
    "textlist[1,1.75;1.3,1;sort_type;Name,Distance;"..sortdropdown.."]"..
    "label[2.4,1.75;Dist:]"..
    "textlist[3,1.75;.5,1;distance_type;3d,2d;"..distdropdown.."]"..
    checkboxes..
    "textlist[0,3.0;9,6;bookmark_list;"..list..";"..bkmrkidx.."]"..
		"button[0,9.3;3.5,1;find_bookmark;Find selected bookmark]"..
    telebutton..
		 "button_exit[8.1,0;1,0.8;button1;ESC]"
...
--	local player = minetest.get_player_by_name(name)
local posx=hud_default_x
local posy=hud_default_y
local color=hud_default_color
if name~=nil then
if hud_pos[name]~=nil then
posx=hud_pos[name].x
posy=hud_pos[name].y
end
if hud_color[name] then
color=hud_color[name]
end
end
	return "compassgps:settings", "size[9,3;]"..
    "button[1,0.2;2.25,1;hud_pos;Change hud:]"..
    "field[3.6,0.5;1.2,1;hudx;X:("..hud_default_x..");"..posx.."]"..
    "field[4.8,0.5;1.2,1;hudy;Y:("..hud_default_y..");"..posy.."]"..
	  "field[6.0,0.5;2,1;hudcolor;Color:("..hud_default_color..");"..color.."]"..
	 "button_exit[1,2;1,0.8;button1;ESC]"
...
for i=1,12 do
  for c,ctype in pairs({"c"}) do
  	local inv = 1
	local gr={not_in_creative_inventory=1}
  	if i == 1 and ctype=="c" then
  		gr = {}
  	end
    ctypename="c"..tostring(i-1)
