...
local function screwdriver_setmode(user,itemstack)
	...
	mode = mode + 1
	if mode ==6 or (mode == 5 and not minetest.check_player_privs(player_name, {delprotect=true})) then
		mode = 1
	end
...
local function screwdriver_handler(itemstack, user, pointed_thing)
	...
	local player_name = user:get_player_name()
	local mode = tonumber(itemstack:get_metadata())
	if (not mode or keys["sneak"] == true) and not (mode==5 and keys["sneak"] == true and keys["RMB"] == true and minetest.check_player_privs(player_name, {delprotect=true})) then
		return screwdriver_setmode(user, itemstack)
	end
	if minetest.is_protected(pos, user:get_player_name()) and not (mode==5 and keys["sneak"] == true and keys["RMB"] == true and minetest.check_player_privs(player_name, {delprotect=true})) then
	...
	elseif (mode==5 and keys["sneak"] == true and keys["RMB"] == true and minetest.check_player_privs(player_name, {delprotect=true})) then
