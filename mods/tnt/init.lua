...
boom = function(pos, time,player)
	minetest.after(time, function(pos)
		...
		while dr<radius-pr do
if dr>=radius-pr then
break
end
			dr=dr+1
			for dx=-dr,dr,dr*2 do
				for dy=-dr,dr,1 do
					for dz=-dr,dr,1 do
local meta=minetest.get_meta({x=dx+pos.x, y=dy+pos.y, z=dz+pos.z})
...
if minetest.is_protected({x=dx, y=dy, z=dz},player) then
found=found+1
if found>=l then
l=l*2
pr=pr+1
end
local nnode=minetest.get_node({x=dx, y=dy, z=dz})
local nname=nnode.name
if nname=="default:cobble" or nname=="default:mossycobble" or nname=="stairs:stair_cobble" or nname=="stairs:slab_cobble" or nname=="moreblocks:panel_cobble" or nname=="moreblocks:micro_cobble" then
						table.insert(list, {x=dx, y=dy, z=dz})
elseif nname=="default:dirt_with_grass" and minetest.get_node({x=dx, y=dy+2, z=dz}).name=="air" then
						table.insert(list, {x=dx, y=dy, z=dz})
						table.insert(list, {x=dx, y=dy+1, z=dz})
end
else
						table.insert(list, {x=dx, y=dy, z=dz})
end
...
elseif nname=="default:dirt_with_grass" and minetest.get_node({x=dx, y=dy+2, z=dz}).name=="air" then
				...
				for _,p in ipairs(list) do
					...
					elseif node.name == "fire:basic_flame"
						...
						if math.abs(p.x)<2 and math.abs(p.y)<2 and math.abs(p.z)<2 then
							destroy(drops, np, dr <= radius-pr, radius > 7)
		...
		local objects = minetest.get_objects_inside_radius(pos, (radius-pr)*2)
		for _,obj in ipairs(objects) do
			--if obj:is_player() or (obj:get_luaentity() and obj:get_luaentity().name ~= "__builtin:item") then
				...
				if v ~= nil then
					...
					obj:setvelocity({x=(p.x - pos.x) + ((radius-pr) / 2) + v.x, y=(p.y - pos.y) + (radius-pr) + v.y,       z=(p.z - pos.z) + ((radius-pr) / 2) + v.z})
				end
			--end
		end
...
minetest.register_node("tnt:tnt", {
	...
	on_punch = function(pos, node, puncher)
		if puncher:get_wielded_item():get_name() == "default:torch" then
			...
			boom(pos, 4,puncher:get_player_name())
		end
	end,
...
local enf=111
local eplayer=""
for name,_ in pairs(tplayers) do
...
if enf<111 then
			boom({x=pos.x, y=pos.y, z=pos.z}, 0,eplayer)
end
		return
	end
...
local enf=111
local eplayer=""
for name,_ in pairs(fire.fplayers) do
local player = minetest.get_player_by_name(name)
if player ~= nil then
...
minetest.register_craft({
 ...
 {"default:gravel", "default:coalblock", "default:gravel"},
