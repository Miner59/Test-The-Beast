...
function cart:on_punch(puncher, time_from_last_punch, tool_capabilities, direction)
	...
	local whois=puncher:get_player_name()	
	if whois==nil then
		return
	end
	if puncher:get_player_control().sneak then
		...
		if minetest.setting_getbool("creative_mode") or (whois~="singleplayer" and not minetest.check_player_privs(whois, {settime=true})) then 
--only allow one cart for all players except admins to prevent players from taking to much carts
			if not inv:contains_item("main", cart) and not inv:contains_item("craft", cart) then
				inv:add_item("main", cart)
				
			elseif not minetest.setting_getbool("creative_mode") then
			minetest.chat_send_player(whois, "You are not allowed to take more carts!")
			end
		else
			inv:add_item("main", cart)
		end
...
function cart:on_step(dtime)
	...
	if not monorail_func.v3:equal( self.current_pos, new_pos ) then
		...
		if minetest.get_node(self.current_pos).name == "monorail:powerrail" or minetest.get_node(self.current_pos).name == "monorail:meserail" or minetest.get_node(self.current_pos).name == "monorail:brakerail" then
...
minetest.register_node("monorail:meserail", { --like powerrail, but always powered and with a bit more acceleration
	description = "MESE Rail",
	drawtype = "raillike",
	tiles = {"monorail_rail_mese.png", "monorail_rail_curved_mese.png", "monorail_rail_t_junction_mese.png", "monorail_rail_crossing_mese.png"},
	inventory_image = "monorail_rail_mese.png",
	wield_image = "monorail_rail_mese.png",
	paramtype = "light",
	is_ground_content = true,
	walkable = false,
	selection_box = {
	...
	after_place_node = function(pos, placer, itemstack)
		...
		minetest.get_meta(pos):set_string("cart_acceleration", "2.32")
	end,
})
