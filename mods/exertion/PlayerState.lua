...
if self.state.fed>12 and self.state.hydrated>2 and self.state.poisoned<1 then
local hp=math.floor((12/self.state.fed)*((self.state.hydrated+6)/26)*dfn-self.builds/7)
if hp>1 then
      self.player:set_hp(self.player:get_hp() + hp);
      self.state.hpGained = hp;
end
end
...
function PlayerState_ops:addHp(dhp)
   ...
   if dhp < 0 or dhp >= 1.0 then
      local dhpf = dhp % 1.0;
      local dhpn = dhp - dhpf;
	if self.player:get_hp() + dhpn<=0 then
		local pos=self.player:getpos();
		local name=self.player:get_player_name();
		if pos~=nil and name~=nil then
			minetest.log("action",name.." died from poison at (".. tostring(pos.x)..","..tostring(pos.y)..","..tostring(pos.z)..")")
		end
	end
...
if player==nil or player:get_breath()==nil then return false end
...
for i,stack in ipairs(player:get_inventory():get_list("main")) do
	if string.sub(stack:get_name(), 0, 19) == "kpgmobs:bucket_milk" then
self.state.hydrated=4
self.state.fed=self.state.fed+1
if stack:get_count()<2 then
player:get_inventory():set_stack("main",i,"bucket:bucket_empty")
else
player:get_inventory():set_stack("main",i,ItemStack(stack:get_name().." "..stack:get_count()))
player:get_inventory():add_item("main","bucket:bucket_empty")
end
	elseif string.sub(stack:get_name(), 0, 22) == "moretrees:coconut_milk" then
self.state.hydrated=3
self.state.fed=self.state.fed+2
if stack:get_count()<2 then
player:get_inventory():set_stack("main",i,"vessels:drinking_glass")
else
player:get_inventory():set_stack("main",i,ItemStack(stack:get_name().." "..stack:get_count()))
player:get_inventory():add_item("main",i,"vessels:drinking_glass")
end
end
if self.state.fed>20 then
self.state.fed=20
end
end
end
	...
	if self.state.hydrated <2 then
if player~=nil then
if player:get_inventory()~=nil then
for i,stack in ipairs(player:get_inventory():get_list("main")) do
	if string.sub(stack:get_name(), 0, 19) == "bucket:bucket_water" then
self.state.hydrated=self.state.hydrated+6
if stack:get_count()<2 then
player:get_inventory():set_stack("main",i,"bucket:bucket_empty")
else
player:get_inventory():set_stack("main",i,ItemStack(stack:get_name().." "..stack:get_count()))
...
if jm>1.0 then jm=1.0 end
   if players[self.player:get_player_name()]~=nil then
	if players[self.player:get_player_name()].boots==nil then
...
function PlayerState_ops:updateHud()
   ...
   local dfd=self.state.fed
   if dfd>12 then dfd=12 end
   if not fh then
      ...
      player:hud_change(fh, 'number', dfd);
   end;

   local hh = self.hydratedHudId;
   local dfd=self.state.hydrated
   if dfd>12 then dfd=12 end
   if not hh then
      ...
      player:hud_change(hh, 'number', dfd);
   end;
